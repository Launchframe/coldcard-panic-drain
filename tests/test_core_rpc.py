...
from coldcard_panic_drain.network_guard import NetworkBlockedError, _is_allowed_host, disable_onion_connections
...
def test_onion_init_failure_does_not_leave_guard_enabled(tmp_path: Path):
    disable_onion_connections()
    missing = tmp_path / "no-cookie.txt"
    with pytest.raises(ValueError, match="Cannot read RPC cookie file"):
        CoreRpcClient(
            f"http://{ONION_HOST}:8332",
            cookie_file=missing,
            allow_onion=True,
        )
    assert not _is_allowed_host(ONION_HOST)
...
def test_network_guard_surfaces_as_core_rpc_error():
    ...
    with patch.object(client._opener, "open", side_effect=NetworkBlockedError("blocked")):
        with pytest.raises(CoreRpcError, match="RPC blocked by network guard"):
